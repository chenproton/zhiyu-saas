...
func (h *OrderHandler) fetchOrderDetail(ctx context.Context, id string) (domain.Order, *OrderDetailResponse, error) {
	...
	_ = h.DB.QueryRow(ctx, `SELECT id, institution_id, name, intro, category, cover_image, attachment, attachment_name, price, version, status, reject_reason, sales_count, view_count, created_at, updated_at FROM resources WHERE id = $1`, order.ResourceID).Scan(

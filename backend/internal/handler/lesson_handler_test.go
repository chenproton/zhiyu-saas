...
func TestCourseBatch_CRUD(t *testing.T) {
	...
	w := env.Do("POST", "/api/v1/lesson/batches", map[string]interface{}{
		"name": "Lesson Batch",
	})
	if w.Code != http.StatusCreated {
		t.Fatalf("expected 201, got %d: %s", w.Code, testhelper.ErrMsg(w))
	}
	batch, err := testhelper.Unmarshal[domain.LessonBatch](w)
	if err != nil {
		t.Fatalf("unmarshal batch: %v", err)
	}
	defer env.DB.Exec(ctx, "DELETE FROM lesson_batches WHERE id = $1", batch.ID)

	if batch.Name != "Lesson Batch" {
		t.Fatalf("expected 'Lesson Batch', got %s", batch.Name)
	}

	w = env.Do("GET", "/api/v1/lesson/batches", nil)
	if w.Code != http.StatusOK {
		t.Fatalf("expected 200, got %d", w.Code)
	}
	items, _, err := testhelper.UnmarshalList[domain.LessonBatch](w)
	if err != nil {
		t.Fatalf("unmarshal list: %v", err)
	}
	if len(items) < 1 {
	...
	w = env.Do("GET", "/api/v1/lesson/batches/"+batch.ID, nil)
	if w.Code != http.StatusOK {
		t.Fatalf("expected 200, got %d", w.Code)
	}
	got, err := testhelper.Unmarshal[domain.LessonBatch](w)
	if err != nil {
		t.Fatalf("unmarshal: %v", err)
	}
	if got.ID != batch.ID {
		t.Fatalf("expected id %s, got %s", batch.ID, got.ID)
	}

	w = env.Do("PUT", "/api/v1/lesson/batches/"+batch.ID, map[string]interface{}{
		"name": "Lesson Batch Updated",
	})
	if w.Code != http.StatusOK {
		t.Fatalf("expected 200, got %d: %s", w.Code, testhelper.ErrMsg(w))
	}
	updated, err := testhelper.Unmarshal[domain.LessonBatch](w)
	if err != nil {
		t.Fatalf("unmarshal updated: %v", err)
	}
	if updated.Name != "Lesson Batch Updated" {
		t.Fatalf("expected 'Lesson Batch Updated', got %s", updated.Name)
	}

	w = env.Do("POST", "/api/v1/lesson/batches/"+batch.ID+"/status", map[string]string{
		"status": "closed",
	})
	if w.Code != http.StatusOK {
		t.Fatalf("expected 200 for status update, got %d: %s", w.Code, testhelper.ErrMsg(w))
	}
	statusUpdated, err := testhelper.Unmarshal[domain.LessonBatch](w)
	if err != nil {
		t.Fatalf("unmarshal: %v", err)
	}
	if statusUpdated.Status != domain.LessonBatchStatusClosed {
		t.Fatalf("expected closed, got %s", statusUpdated.Status)
	}

	w = env.Do("DELETE", "/api/v1/lesson/batches/"+batch.ID, nil)
	if w.Code != http.StatusOK {
		t.Fatalf("expected 200, got %d", w.Code)
	}

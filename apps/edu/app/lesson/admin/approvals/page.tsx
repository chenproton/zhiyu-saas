...
export default function CourseApprovalsPage() {
  const { records, loading, approve, reject } = useApprovals({ targetType: "course" })
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-slate-50">
                ...
                <TableHead className="text-xs font-medium text-slate-500 whitespace-nowrap">专业</TableHead>
                <TableHead className="text-xs font-medium text-slate-500 whitespace-nowrap">所属批次</TableHead>
                  ...
                  <TableRow key={item.id}>
                    ...
                    <TableCell className="text-center text-sm text-gray-600 whitespace-nowrap">
                      <Badge variant="secondary" className="px-1.5 py-0 text-xs">
                        {COURSE_TYPE_LABELS[item.courseType] || item.courseType}
                      </Badge>
                    </TableCell>
                    ...
                    <TableCell className="text-right whitespace-nowrap sticky right-0 bg-white z-10 shadow-[-4px_0_8px_-4px_rgba(0,0,0,0.05)]">
                      <div className="flex items-center justify-end gap-2">
                        <Button variant="outline" size="sm" asChild>
                          <Link href={`/lesson/admin/courses/${item.courseId}/edit`}>
      ...
      <Tabs defaultValue="pending">
        ...
        <TabsContent value="pending" className="mt-6">
          {pendingItems.length > 0 ? renderTable(pendingItems) : (
            <Card><CardContent className="py-12 text-center"><CheckSquare className="h-12 w-12 text-gray-300 mx-auto mb-4" /><h3 className="text-lg font-medium text-gray-700">暂无待审批项</h3><p className="text-sm text-gray-500 mt-1">所有提交的课程都已处理完毕</p></CardContent></Card>
          )}
        </TabsContent>

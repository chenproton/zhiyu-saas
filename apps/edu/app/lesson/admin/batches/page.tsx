...
export default function BatchesPage() {
  ...
  const handleUpdateBatch = async () => {
    ...
    try {
      await lessonBatchApi.update(editingBatch.id, {
        name: newBatchName,
  ...
  const handleDeleteBatch = async (id: string) => {
    ...
    try {
      await lessonBatchApi.delete(id)
      await loadData()
      toast({ title: "删除成功" })
    } catch (err: any) {
  ...
  const renderForm = (isEdit: boolean) => (
    <div className="grid gap-4 py-4">
      ...
      <div className="grid gap-2">
        ...
        <p className="text-xs text-gray-500">批次内所有课程将强制使用相同的审批流程</p>
      </div>
    </div>
  )
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">管理课程建设批次分组，关联审批流程</p>
        </div>
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-slate-50">
                  ...
                  <TableHead className="text-xs font-medium text-slate-500">体系课数</TableHead>
                ...
                {loading ? (
                  ...
                  batches.map((batch) => (
                    <TableRow key={batch.id}>
                      ...
                      <TableCell className="text-sm text-gray-600">{batch.courseCount ?? 0}</TableCell>

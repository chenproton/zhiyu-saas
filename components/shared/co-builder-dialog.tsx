...
// 按部门组织用户
const departmentTree = mockDepartments.map((dept) => ({
  name: dept.name,
  users: mockUsers.filter((u) => u.department === dept.name),
...
export function CoBuilderDialog({
  ...
  const selectedUsers = mockUsers.filter((t) => selectedIds.includes(t.id))
